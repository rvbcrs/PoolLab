...
#define EXAMPLE_PIN_NUM_BK_LIGHT GPIO_NUM_2  // Match ESP32P4_Display backlight control
...
{   
    Serial.println("st7701_lcd::begin() - START");
    Serial.flush();
    
    Serial.println("st7701_lcd::begin() - Calling example_bsp_enable_dsi_phy_power()");
    Serial.flush();
    example_bsp_enable_dsi_phy_power();
    Serial.println("st7701_lcd::begin() - DSI PHY power enabled");
    Serial.flush();
    
    Serial.println("st7701_lcd::begin() - Calling example_bsp_init_lcd_backlight()");
    Serial.flush();
    example_bsp_init_lcd_backlight();
    Serial.println("st7701_lcd::begin() - Backlight initialized");
    Serial.flush();
    // example_bsp_set_lcd_backlight(EXAMPLE_LCD_BK_LIGHT_OFF_LEVEL);

    // 首先创建 MIPI DSI 总线，它还将初始化 DSI PHY
    Serial.println("st7701_lcd::begin() - Creating MIPI DSI bus");
    Serial.flush();
    esp_lcd_dsi_bus_handle_t mipi_dsi_bus;
    esp_lcd_dsi_bus_config_t bus_config = ST7701_PANEL_BUS_DSI_2CH_CONFIG();
    Serial.println("st7701_lcd::begin() - Calling esp_lcd_new_dsi_bus()");
    Serial.flush();
    ESP_ERROR_CHECK(esp_lcd_new_dsi_bus(&bus_config, &mipi_dsi_bus));
    Serial.println("st7701_lcd::begin() - MIPI DSI bus created");
    Serial.flush();

    ESP_LOGI(TAG, "Install MIPI DSI LCD control panel");
    // 我们使用DBI接口发送LCD命令和参数
    Serial.println("st7701_lcd::begin() - Creating DBI config");
    Serial.flush();
    esp_lcd_dbi_io_config_t dbi_config = ST7701_PANEL_IO_DBI_CONFIG();
    Serial.println("st7701_lcd::begin() - Calling esp_lcd_new_panel_io_dbi()");
    Serial.flush();
    ESP_ERROR_CHECK(esp_lcd_new_panel_io_dbi(mipi_dsi_bus, &dbi_config, &io_handle));
    Serial.println("st7701_lcd::begin() - Panel IO DBI created");
    Serial.flush();

    // 创建JD9365控制面板
    Serial.println("st7701_lcd::begin() - Creating DPI panel config");
    Serial.flush();
    esp_lcd_dpi_panel_config_t dpi_config = ST7701_480_360_PANEL_60HZ_DPI_CONFIG(MIPI_DPI_PX_FORMAT);

    Serial.println("st7701_lcd::begin() - Creating vendor config");
    Serial.flush();
   st7701_vendor_config_t vendor_config = {
    ...
    Serial.println("st7701_lcd::begin() - Creating panel config");
    Serial.flush();
    const esp_lcd_panel_dev_config_t panel_config = {
        .reset_gpio_num = (gpio_num_t)_lcd_rst,
        .rgb_ele_order = LCD_RGB_ELEMENT_ORDER_RGB,
        .bits_per_pixel = 16,
        .vendor_config = &vendor_config,
    };
    Serial.println("st7701_lcd::begin() - Calling esp_lcd_new_panel_st7701()");
    Serial.flush();
    ESP_ERROR_CHECK(esp_lcd_new_panel_st7701(io_handle, &panel_config, &panel_handle));
    Serial.println("st7701_lcd::begin() - ST7701 panel created");
    Serial.flush();
    
    Serial.println("st7701_lcd::begin() - Calling esp_lcd_panel_reset()");
    Serial.flush();
    ESP_ERROR_CHECK(esp_lcd_panel_reset(panel_handle));
    Serial.println("st7701_lcd::begin() - Panel reset complete");
    Serial.flush();
    
    Serial.println("st7701_lcd::begin() - Calling esp_lcd_panel_init()");
    Serial.flush();
    // TEMPORARILY SKIP - this is where it crashes
    Serial.println("st7701_lcd::begin() - SKIPPING esp_lcd_panel_init() for debugging");
    Serial.flush();
    //ESP_ERROR_CHECK(esp_lcd_panel_init(panel_handle));
    //Serial.println("st7701_lcd::begin() - Panel init complete");
    //Serial.flush();
    ...
    // 打开背光
    Serial.println("st7701_lcd::begin() - Setting backlight");
    Serial.flush();
    example_bsp_set_lcd_backlight(EXAMPLE_LCD_BK_LIGHT_ON_LEVEL);
    Serial.println("st7701_lcd::begin() - COMPLETE");
    Serial.flush();
}
